...
def calc_TP_onlySS(info_ALLState_df:pd.DataFrame, diag_value:Union[int, str], 
    ...
    #----------------------
    # df -> Graoh
    # Add edges (vertices will be added as needed)
    ...
    for _, E_use in enumerate(E_list):
        ...
        for _, state1_use in enumerate(state_list):
            for _, state2_use in enumerate(state_list):
                if TP_onlySS_df.loc[state1_use, state2_use] is None: # Only if 'E' has not yet been registered in TP_onlySS_df
                    if nx.has_path(G=G, source=state1_use, target=state2_use) == False: # Add E to df_TP when a path disappears for the first time (split into two ifs for speed)
                        ...
                        if print_progress!=False:
                            print(i)
    #----------------------
    # TP_onlySS_df : Fill the diagonal with "E" or "0"
    ...
    # TP_onlySS_name_df : Fill the diagonal with "state"
    for _, state_use in enumerate(state_list):
