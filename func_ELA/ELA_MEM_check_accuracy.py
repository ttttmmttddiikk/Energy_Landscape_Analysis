...
def plot_model_empirical(binarizedData_df:pd.DataFrame, h_1darray:np.ndarray, J_2darray:np.ndarray, 
    ...
    #-----------
    #-----------
    # Convert all values in binarizedData_df to integers
    binarizedData_df = binarizedData_df.astype('int')
    #-----------
    # datas_df (e.g. state: 0,1,1 -> 011)
    ...
    # plot r I2/IN
    plt.text(x=0.99, y=3*0.1, s="$n = {}$".format(len(binarizedData_df)), fontdict=dict(fontsize=40,fontstyle="italic"), va='top', ha='right', transform=ax.transAxes)
    plt.text(x=0.99, y=2*0.1, s="$r = {}$".format("{:.5f}".format(r)), fontdict=dict(fontsize=40,fontstyle="italic"), va='top', ha='right', transform=ax.transAxes)
    plt.text(x=0.99, y=1*0.1, s="$I_2/I_N = {}$".format("{:.5f}".format(I2_IN)), fontdict=dict(fontsize=40,fontstyle="italic"), va='top', ha='right', transform=ax.transAxes)
    # setting
...
def return_KL_div(Px_1darray:np.ndarray, Qx_1darray:np.ndarray) -> int: # Px_1darray: e.g. Px_1darray=[3/5, 1/5, 1/5]
    KL_div = 0
